...
RCT_EXPORT_METHOD(trackWithRNDynamicProperty:(NSString *)event withProperties:(NSDictionary *)propertyDict dynamicProperty:(NSDictionary*)dynamicProperty){
    NSLog(@"%@",dynamicProperty);
    NSMutableDictionary *mutDict = [NSMutableDictionary dictionaryWithDictionary:propertyDict];
    [propertyDict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:NSArray.class]) {
            [mutDict setObject:[NSSet setWithArray:obj] forKey:key];
    ...
    [[SensorsAnalyticsSDK sharedInstance] track:event withProperties:propertyDict reactNativeDynamicProperties:dynamicProperty];
